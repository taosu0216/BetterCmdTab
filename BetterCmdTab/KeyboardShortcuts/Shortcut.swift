...
extension SpecialKey {
	fileprivate var presentableDescription: String {
		...
		case .home:
			"↖"
		case .space:
			String(localized: "space_key", table: "Shortcuts").capitalized // This matches what macOS uses.
		case .tab:
			"⇥"
		case .pageUp:
			"⇞"
...
extension KeyboardShortcuts.Shortcut: CustomStringConvertible {
	...
	*/
	...
	nonisolated public var description: String {
		// CustomStringConvertible requires nonisolated description
		// Use static key mapping to avoid MainActor.assumeIsolated
		let modifierString = modifiers.ks_symbolicRepresentation
		let keyString = Self.keyCodeToString(carbonKeyCode)
		return modifierString + keyString
	}
	
	/// Static mapping of key codes to string representation - safe to call from any thread
	private nonisolated static func keyCodeToString(_ keyCode: Int) -> String {
		// Special keys mapping based on Carbon key codes
		switch keyCode {
		// Letters A-Z (Carbon ANSI key codes)
		case 0: return "A"
		case 11: return "B"
		case 8: return "C"
		case 2: return "D"
